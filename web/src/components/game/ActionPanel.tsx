...
export function ActionPanel({
  ...
  return (
    <div className="space-y-3">
      ...
      <div>
        ...
        <div className="flex gap-2 overflow-x-auto pb-1">
          ...
          <button
            ...
            className={clsx(
              'flex-shrink-0 px-3 py-2 rounded-lg text-sm font-display',
          ...
          {sortedQuickActions.map(({ action, config }) => {
            ...
            return (
              <button
                ...
                disabled={disabled || isExecuting || turnsRemaining < 1}
                className={clsx(
                  'flex-shrink-0 px-3 py-2 rounded-lg text-sm font-display border',
      ...
      <div>
        <h3 className="text-label mb-2">Turn Action</h3>
        <div className="grid grid-cols-5 gap-1">
          {ACTIONS.map((config) => (
            ...
            >
              ...
              <span className="text-xs font-display uppercase tracking-wider mt-1">
                {config.label}
              </span>
            </button>
          ))}
      ...
      <div className="bg-game-card rounded-lg border border-game-border p-3 space-y-3">
        ...
        <div className="flex items-center gap-3">
          <span className="text-gray-500 text-sm w-12">Turns:</span>
          <div className="flex items-center gap-1 flex-1">
            <button
              onClick={() => setTurns(Math.max(1, turns - 10))}
              disabled={disabled || isExecuting || turns <= 1}
              className="w-10 h-10 rounded bg-game-dark border border-game-border text-red-400 disabled:opacity-30"
            >
              -10
            </button>
            <button
            ...
            <input
              ...
              disabled={disabled || isExecuting}
              className="w-16 h-10 text-center font-stats text-lg bg-game-dark border border-game-border rounded"
            />
            <button
              onClick={() => setTurns(Math.min(turnsRemaining, turns + 1))}
              disabled={disabled || isExecuting || turns >= turnsRemaining}
              className="w-10 h-10 rounded bg-game-dark border border-game-border text-green-400 disabled:opacity-30"
            >
              +1
            </button>
            <button
          ...
          <button
            ...
            disabled={disabled || isExecuting}
            className="px-3 h-10 rounded bg-game-dark border border-game-border text-cyan-400 text-sm"
          >
            Max
          </button>
        </div>
        ...
        {selectedAction === 'industry' && (
          <div className="pt-2 border-t border-game-border space-y-2">
            <div className="flex justify-between items-center">
              <span className="text-sm text-gray-400">Troop Allocation:</span>
              <span className={clsx('font-stats text-sm', isAllocationValid ? 'text-green-400' : 'text-red-400')}>
                {allocationTotal}%
              </span>
            </div>
            {(['trparm', 'trplnd', 'trpfly', 'trpsea'] as const).map((type) => {
              const names = { trparm: 'Infantry', trplnd: 'Cavalry', trpfly: 'Aircraft', trpsea: 'Navy' };
              return (
                <div key={type} className="flex items-center gap-2">
                  <span className="text-gray-400 text-sm w-16">{names[type]}</span>
                  <button
                    onClick={() => setIndustryAllocation(prev => ({ ...prev, [type]: Math.max(0, prev[type] - 10) }))}
                    className="w-8 h-8 rounded bg-game-dark border border-game-border text-red-400"
                  >
                    -
                  </button>
                  <div className="flex-1 relative h-6">
                    ...
                    <span className="absolute inset-0 flex items-center justify-center text-sm font-stats">
                      {industryAllocation[type]}%
                    </span>
                  </div>
                  <button
                    onClick={() => setIndustryAllocation(prev => ({ ...prev, [type]: Math.min(100, prev[type] + 10) }))}
                    className="w-8 h-8 rounded bg-game-dark border border-game-border text-green-400"
                  >
                    +
                  </button>
                </div>
        ...
        <button
          ...
          className={clsx(
            'w-full py-3 rounded-lg font-display uppercase tracking-wider transition-all',
        ...
        >
          {isExecuting ? 'Executing...' : `${selectedConfig.icon} ${selectedConfig.label} (${turns} turns)`}
        </button>
      </div>

      {/* Complex Actions */}
      <div>
        <h3 className="text-label mb-2">Complex Actions</h3>
        <div className="grid grid-cols-3 gap-2">
          ...
          >
            <span className="text-xl">⚔️</span>
            <span className="text-xs font-display">Attack</span>
          ...
          >
            ...
            <span className="text-xs font-display">Spell</span>
            <span className="text-xs text-gray-500">2T</span>
          </button>
        </div>
      </div>
      ...
      <div className="grid grid-cols-4 gap-1">
        <button onClick={onMarket} disabled={disabled} className="btn-ghost text-xs py-2">🏪 Market</button>
        <button onClick={onBank} disabled={disabled} className="btn-ghost text-xs py-2">🏦 Bank</button>
        <button onClick={onEnemies} disabled={disabled} className="btn-ghost text-xs py-2">👁️ Enemies</button>
        <button onClick={onOverview} disabled={disabled} className="btn-ghost text-xs py-2">📊 Overview</button>
      </div>
      ...
      <button
        ...
        disabled={disabled || isExecuting}
        className={clsx(
          'w-full py-3 rounded-lg font-display uppercase tracking-wider text-sm',
      ...
      <div className="flex justify-between text-xs">
        <button onClick={onGuide} className="text-gray-500 hover:text-gray-300">📖 Guide</button>
        <button onClick={onAbandon} className="text-gray-500 hover:text-red-400">🏳️ Abandon</button>
      </div>
    </div>
  );
}
